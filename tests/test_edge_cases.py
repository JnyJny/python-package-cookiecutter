...
def test_no_license_project(
    ...
    assert license_path.exists(), "LICENSE file should exist even for no-license projects"
    
    license_content = license_path.read_text()
    assert "public domain" in license_content, "Should contain Unlicense text"
    
    # Check rest of project structure
...
def test_no_pydantic_settings_project(
    ...
    settings_path = Path(project_path) / "src" / cookiecutter_package_name / "settings.py"
    assert not settings_path.exists(), "settings.py should not exist when pydantic-settings is disabled"
    ...
    assert check_project_contents(project_path, cookiecutter_package_name, basic_context)
...
def test_generated_project_imports_work(generated_template_path: Path) -> None:
    ...
    result = subprocess.run(
        ["uv", "run", "python", "-c", "import thing; import thing.__main__; import thing.self_subcommand"],
