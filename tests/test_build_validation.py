...
class TestBuildValidation:
    ...
    def test_wheel_package_structure(
        ...
        with zipfile.ZipFile(wheel_file, 'r') as wheel:
            ...
            metadata_files = [f for f in wheel_contents if f.endswith(".dist-info/METADATA")]
            ...
            entry_point_files = [f for f in wheel_contents if f.endswith(".dist-info/entry_points.txt")]
            if entry_point_files:
                # Read entry points
                with wheel.open(entry_point_files[0]) as ep_file:
                    entry_points = ep_file.read().decode('utf-8')
                    assert "[console_scripts]" in entry_points, "Should have console scripts"
    ...
    def test_source_distribution_structure(
        ...
        with tarfile.open(sdist_file, 'r:gz') as sdist:
            ...
            src_files = [f for f in sdist_contents if "/src/" in f and f.endswith(".py")]
            assert len(src_files) > 0, "Source distribution should contain source files"
            
            # Should contain pyproject.toml
            pyproject_files = [f for f in sdist_contents if f.endswith("pyproject.toml")]
    ...
    def test_package_metadata_validity(
        ...
        with zipfile.ZipFile(wheel_files[0], 'r') as wheel:
            ...
            with wheel.open(metadata_files[0]) as metadata_file:
                ...
                assert "Author:" in metadata or "Author-Email:" in metadata, "Should have author info"
    ...
    def test_package_installation_from_wheel(
        ...
        assert result.returncode == 0, f"Virtual environment creation failed: {result.stderr}"
        ...
        if venv_python.exists():
            ...
            assert result.returncode == 0, f"Installed package import failed: {result.stderr}"
    ...
    def test_build_reproducibility(
        ...
        assert first_files.keys() == second_files.keys(), "Build artifacts should be the same"
        
        for filename in first_files:
            assert first_files[filename] == second_files[filename], f"File {filename} size differs between builds"
    ...
    def test_build_with_different_backends(
        ...
        for backend in backends_to_test:
            ...
            assert result.returncode == 0, f"Build with {backend} backend failed: {result.stderr}"
    ...
    def test_package_size_reasonable(
        ...
        for wheel_file in wheel_files:
            ...
            assert wheel_size < 1024, f"Wheel {wheel_file.name} is too large: {wheel_size:.1f}KB"
            assert wheel_size > 1, f"Wheel {wheel_file.name} is suspiciously small: {wheel_size:.1f}KB"
        
        # Check sdist size
        sdist_files = list(dist_dir.glob("*.tar.gz"))
        for sdist_file in sdist_files:
            sdist_size = sdist_file.stat().st_size / 1024  # KB
            assert sdist_size < 2048, f"Sdist {sdist_file.name} is too large: {sdist_size:.1f}KB"
            assert sdist_size > 1, f"Sdist {sdist_file.name} is suspiciously small: {sdist_size:.1f}KB"
    ...
    def test_version_consistency(
        ...
        with zipfile.ZipFile(wheel_files[0], 'r') as wheel:
            ...
            with wheel.open(metadata_files[0]) as metadata_file:
                ...
                assert pyproject_version == metadata_version, "Versions should match between pyproject.toml and wheel metadata"